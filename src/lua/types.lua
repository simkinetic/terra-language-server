...
T.Type.__tostring = memoizefunction(function(self)
    ...
    elseif self:isfunction() then return ast.mkstring(self.parameters,"{",",",self.isvararg and " ...}" or "}").." -> "..tostring(self.returntype)
    ...
    getvalue = function(self)
        ...
        local function checkentry(e,results)
            if type(e) == "table" then
                ...
                if types.istype(t) and (type(f) == "string" or ast.islabel(f)) then
                    results:insert { type = t, field = f}
                    return
                elseif ast.israwlist(e) then
                    local union = List()
            ...
            diagnostics.erroratlocation(self.anchor,"expected either a field type pair (e.g. { field = <string>, type = <type> } or {<string>,<type>} ), or a list of valid entries representing a union")
        end
...
local function reportopaque(type)
    ...
    if type.anchor then
        diagnostics.erroratlocation(type.anchor,msg)
    else
        error(msg,4)
    end
end
    ...
    getvalue = function(self)
        ...
        local function addentry(k,t)
            ...
            if layout.keytoindex[entry.key] ~= nil then
                diagnostics.erroratlocation(tree,"duplicate field ",tostring(entry.key))
            end
...
local function globaltype(name, typ, min_v, max_v)
    ...
    -- We removed rawset(_G, name, typ) to prevent global namespace pollution
    types[name] = typ
    -- Just return the raw value for the LSP, no ffi.cast needed!
...
function types.funcpointer(parameters,ret,isvararg)
    ...
    if not types.istype(ret) and ast.israwlist(ret) then
...
function types.cast(terratype, obj)
    -- We removed terratype:complete() and cstring()
    -- Just use the standard LuaJIT cast
